...
class Self_Attention(nn.Module):
    def __init__(self, hidden_dim):
        ...
        # Define three linear transformations for query, key, and value
        self.query_layer = nn.Linear(hidden_dim * 2, hidden_dim * 2)
        self.key_layer = nn.Linear(hidden_dim * 2, hidden_dim * 2)
        self.value_layer = nn.Linear(hidden_dim * 2, hidden_dim * 2)

    def forward(self, lstm_outputs):
        # 为自我关注计算查询、键和值
        queries = self.query_layer(lstm_outputs)
        keys = self.key_layer(lstm_outputs)
        values = self.value_layer(lstm_outputs)
        # 计算按比例点积关注度
        scores = torch.bmm(queries, keys.transpose(1, 2)) / (lstm_outputs.size(-1) ** 0.5)  # 比例因子
        attention_weights = torch.softmax(scores, dim=-1)
        # 对数值应用注意力权重
        weighted_output = torch.bmm(attention_weights, values)
        # 合并各时间步的加权值
...
class CNN_LSTM_Attention_Model(nn.Module):
    def __init__(self, conv_input, input_dim, hidden_dim, num_layers, output_dim):
        ...
        self.conv_activation = nn.LeakyReLU(negative_slope=0.01)
...
optimizer = optim.Adam(model.parameters(), lr=0.001, betas=(0.5, 0.999), weight_decay=0.0001)
