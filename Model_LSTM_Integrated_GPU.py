...
#集成LSTM(CNN-KAN-BID，L1正则化，dropout正则化）
#目前设定状态：
class LSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        #out = self.fc(out[:, -1, :]) #无dropout正则化输出
        out = self.dropout(out[:, -1, :])  # dropout正则化输出
...
model = LSTM(conv_input, input_dim, hidden_dim, num_layers, output_dim).to(device)
